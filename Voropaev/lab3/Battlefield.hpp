...
void Battlefield::Enter(ifstream &fin){ //ввод из файла
    ...
    fin >> size;
    fin >> q;
    shared_ptr<Crown> tmp(new Crown("red"));
    crown_red = tmp;
    Object obj1(tmp);
    for (size_t i = 0; i < q; i++) {
        fin >> obj1;
       // obj1.get_position() = i; //располагаем считываемый объект на i=ой позиции
        red_army.Add_to_Head(obj1);//добавляет объект в красную армию
    }

    fin >> q;
    shared_ptr<Crown> tmp1(new Crown("green"));
    crown_green = tmp1;
    Object obj2(tmp1);
    for (size_t i = 0; i < q; i++) {
        fin >> obj2;
        //obj2.get_position() = i;
        green_army.Add_to_Head(obj2);
    }

    cout << '\n'<< "Destroying interim object : ";
    cout << '\n';
}

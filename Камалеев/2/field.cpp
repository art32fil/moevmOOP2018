...
void Field::Get_Red_Army(ifstream& fl, const shared_ptr<Crown>& color) { // Инициализация бойцов красной армии
	fl >> red_army_size;
	for (int i = 0; i < red_army_size; i++)
		red_army.Supl_End(fl, color);
}

void Field::Get_Green_Army(ifstream& fl, const shared_ptr<Crown>& color) { // Инициализация бойцов зелёной армии
	fl >> green_army_size;
...
istream& operator >>(istream& str, Field& fld){ // оператор >>
 str >> fld.x_size >>fld.y_size;
 return str;
}

ostream& operator <<(ostream& str, const Field& fld){ // оператор <<
 str << fld.x_size << fld.y_size;
 return str;
}

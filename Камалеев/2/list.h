...
public:
	...
	element* prev;
    T info;
	element(const T* info) : info(*info), prev(nullptr), next(nullptr) {}
};
...
public:
   // element<T> *head, *tail;
	...
	public:    
        ...
        Iterator& operator ++() // оператор ++(int)
        {
            ptr = ptr->next;
            return *this;
        };
	...
	void Supl_End(ifstream& fl, const shared_ptr<Crown>& color) // добавление в конец
	{
		T* obj = new T(fl, color);
		element<T> *temp = new element<T>(obj);
		obj->Delete_Crown();
		//temp->info = *info;
		operator delete (obj);
		if (temp == NULL) {

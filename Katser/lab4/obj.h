...
class obj {
	...
	public:	
		...
		obj (const int& a, const int& b, const int& h,std::shared_ptr< Crown > cr) //конструктор
		{
			x=a;
			y=b;
			heal=h;
			crown=cr;
			id=count++;
			std::cout<<"Объект: х="<<x<<" у="<<y<<" hp="<<heal<<" id="<<id<<" id_crown="<<color()<< std::endl;
		}

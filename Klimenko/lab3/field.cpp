...
int battlefield::hit(_2dim pos, int id) {
	...
	if (type == 'w') {
		...
		if (list->capacity() == 1) if (unit->get_damage(archer->give_force())) if (archer->last_representive()) return (archer->show_team() == 1) ? 2 : 1; //suicide is hard work
		for (auto obj : *list) {
			if (obj->get_id() == archer->get_id()) continue;
			if (auto tmp = obj->get_damage(archer->give_force())) {
				if (obj->last_representive()) return archer->show_team();
				del_by_id(tmp);
			}
		}
	}
	else if (type == 'k') {
		Kamikaze* kamikaze = dynamic_cast<Kamikaze *>(unit);
		vector <Object *> *list = select_by_coords(kamikaze->get_coords());
		if (list->capacity() == 1) if (unit->get_damage(kamikaze->give_force())) if (kamikaze->last_representive()) return (kamikaze->show_team() == 1) ? 2 : 1; //suicide is hard work
		for (auto obj : *list) {
			if (obj->get_id() == kamikaze->get_id()) continue;
			if (auto tmp = obj->get_damage(9999999999)) {
				if (obj->last_representive()) return kamikaze->show_team();
				del_by_id(tmp);
			}
		}
	}
